...
| `gen_item_icons.gd` | 아이템 획득 팝업/인벤토리 UI용 자리표시 아이콘(32px 단색 사각형 + 테두리)을 절차적으로 그려서 `assets/props/ui_icons/`에 저장 | 재사용 중 — 새 아이템 아이콘이 필요하면 `_icons` 딕셔너리에 `이름: [칠 색, 테두리 색]` 추가 후 재실행. 실제 아이콘 아트로 교체되면 필요 없어짐(`gen_player_sprite.gd`와 같은 임시 성격) |
