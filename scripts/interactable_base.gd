...
func _process(_delta: float) -> void:
	...
	visible = show_now
	if not show_now or _player == null or DialogueSystem.is_active() or DialogueSystem.just_ended_this_frame() or ItemPopup.is_active() or ItemPopup.just_closed_this_frame() or ChapterTitleCard.is_active() or ChapterTitleCard.just_ended_this_frame():
		if _prompt_label:
			_prompt_label.visible = false
		return
