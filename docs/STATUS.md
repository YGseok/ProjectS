...
- **스토리 진상 대개정**(2026-09-04): `docs/scenario/2026-09-04.md`를 받아
  ...
  **폐기**, 계모와의 몸싸움 중 익사·빨간 연필 금기·필통·일기 중심의 새
  ...
  전제("외갓집에 놀러옴" → 실제로는 본인 고향집)만 고치고, §7.1 메인 퍼즐
  재구성 여부는 한동안 미정 상태로 보류돼 있었음. **2026-09-07 INBOX.md로
  결정 도착, §7.1을 "닫힌 일기장" 퍼즐로 교체 + 실제 구현까지 완료** —
...
- `res://scenes/chapter1_real.tscn` / `chapter1_dream.tscn` — 배경은 이제
  ...
  진짜 트리거 조건(DESIGN.md §7.1)으로 아직 안 바뀌어 있음 — 다음 할 일
  큐 참고.
- **챕터 1 메인 퍼즐 "닫힌 일기장" 구현**(INBOX.md 결정 반영, 2026-09-07):
  DESIGN.md §7.1을 4단계 수집 퍼즐로 교체하고 실제 오브젝트까지 구현.
...
> §7.1 스토리 결정은 2026-09-07 INBOX.md로 도착해 반영 완료(위 "챕터 1
...
- **이터레이션 배치 10/10 — 마무리 회귀 스윕**: `tests/run_all.sh`
  ...
  회귀 테스트 보강, (3) DESIGN.md §9 정확성 점검(스토리 결정 아님,
...
- **이터레이션 배치 3/10 — DESIGN.md §9(미정 사항) 정확성 점검**: §7.1
  ...
  제외). 대신 §9.3에서 실제로 이미 오래전에 답이 난 미정 항목을 하나
  ...
  일관된 실제 구현이었다 — 문서가 코드를 따라가지 못한 사례. §9.3에서
  제거하고, §8의 관련 문장도 "분리될 경우"라는 가정형에서 "실제로 구현돼
  있다"는 사실 서술로 고쳤다.
...
- **챕터 1 메인 퍼즐 "닫힌 일기장" 구현(INBOX.md 스토리 결정 반영,
  ...
  - **4개 오브젝트** (전부 `chapter1_real.tscn`에 배치, 그리드 정렬 좌표):
    ...
    DESIGN.md §7.1의 "딱 한 번만 스치듯" 요구사항).
  ...
  - **미완성으로 남겨둔 부분**: 각성 트리거가 여전히 "Enter만 누르면
    무조건 각성"인 임시 버전 그대로다. DESIGN.md §7.1대로면 4단계 일기
