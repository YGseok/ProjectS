...
**큐 비어있음(v0.31 기준)** — DESIGN.md §11(챕터 2/3 + 일상 NPC),
맵 확장, 풋스위치, 마루/사방치기 아트까지 이전 큐의 모든 항목이 v0.22~
v0.31에 걸쳐 완료됐다(세부 내용은 아래 "완료 기록" 참고). 막혀있던
항목들(탈출 후 단서 콘텐츠, 캐릭터 스프라이트, 마루/사방치기 아트)은
"기능부터, 내용은 나중" 방침(사람 확인, 2026-09-15)에 따라 전부
placeholder로 채워짐 — 각 항목의 "진짜 최종본"(단서 내용, 캐릭터 최종
디자인)은 여전히 사람 확인이 필요하지만, **캐릭터 최종 디자인 확정은
사람이 보류(2026-09-15 "그건 패스")**시켰으므로 다음 지시가 있을 때까지
건드리지 말 것.

새 INBOX.md 지시가 오기 전까지는 기존 기능의 버그 수정/리팩터/테스트
보강 위주로 자가 판단해서 이어갈 것 — 각 작업 후 `qa/run_all.sh`(시각)
+ `tests/run_all.sh`(상호작용)로 검증하고, 사람 눈 확인이 여전히 최종
기준임을 유의.
