...
5. **재미 요소 추가(사람 피드백, 2026-09-08 — "인터렉션 오브젝트나 풋
   스위치 등, 무언가가 더 필요하다")도 아직 미착수**. 구체적으로 어떤
   기믹을 챕터 1 어디에 넣을지는 게임플레이/레벨 디자인 결정이 필요해서
   임의로 배치하지 않음 — 다만 "풋 스위치"류의 범용 메커니즘(밟으면
   신호를 보내는 압력판 등) 자체는 스토리와 무관한 재사용 컴포넌트로
   미리 만들어둘 수는 있음(다음 이터레이션 후보).
