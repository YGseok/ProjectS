...
> **현재 자가 선택으로 진행 가능한 항목이 소진됨(v0.28 기준)**: 위
> 1번(마루/사방치기 실제 아트)은 다른 에셋 팩이나 직접 제작이 필요하고,
> "탈출 후 일상 파트의 단서 콘텐츠"(아래 처리 대기 중 참고)와 플레이어
> 캐릭터 스프라이트는 스토리/아트 결정이 필요해서 전부 사람 확인 대기
> 상태다. 새 INBOX.md 지시가 오기 전까지는 기존 기능의 버그 수정/리팩터/
> 테스트 보강 위주로 이어갈 것.
