...
4. **맵 확장은 `chapter1_real.tscn`(현실)에 v0.23으로 완료**(사람 피드백,
   2026-09-08 + 2026-09-14 "레벨 디자인 및 내부 구조는 임의로 일임한다"
   — 기와집 내부 방 2개, 실제 스크롤링도 처음으로 검증 완료, DESIGN.md
   §8.3/STATUS.md v0.23 참고). **아직 안 한 것**: `chapter1_dream.tscn`
   (꿈)에는 이 확장이 반영 안 됨 — 같은 물리 공간이 현실/꿈 모두에
   반복 등장한다는 DESIGN.md §4 원칙을 지키려면 나중에 꿈 쪽에도 같은
   구조를 반영해야 함. 새 방 안에는 가구/상호작용 오브젝트가 전혀
   없음(순수 이동 공간만 확보) — 필요해지면 채울 것.
