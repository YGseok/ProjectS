...
5. **재미 요소 추가(사람 피드백, 2026-09-08 — "인터렉션 오브젝트나 풋
   스위치 등, 무언가가 더 필요하다")**: 나무/덤불 조사(`scenery_flavor.gd`)
   는 v0.07로 완료(위 완료 기록 참고). **풋 스위치 쪽은 여전히 부품만
   준비되고 배치는 미착수** — `scripts/pressure_plate.gd`(범용 발판
   컴포넌트)를 만들고 테스트까지 마쳤지만 챕터 1의 어떤 씬/퍼즐에도 아직
   안 붙였다. 구체적으로 어디서 어떤 효과(문이 열림, 소리가 남 등)로
   쓸지는 게임플레이/레벨 디자인 결정이 필요해서 사람 확인 후 배치할 것.
