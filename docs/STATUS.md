...
0-a. **일상 파트 NPC 3개는 v0.27로 완료**(INBOX.md 2026-09-14, DESIGN.md
   §11.4). DESIGN.md §11 계획 전체(챕터 2/3 뼈대 + NPC 3개)가 이제
   모두 구현됨 — 아래 완료 기록 참고.
...
1. **`floorboard.gd`(마루 판자)/`hopscotch_key.gd`(사방치기)는 v0.31로
   완료** — 맞는 타일 에셋이 없어서(`assets/tiles/main/` 전체 확인
   완료, 학교·사무 공간용 소재뿐) 직접 제작으로 대체함: 판자는 기존
   마루 타일을 가공, 사방치기는 `_draw()`로 직접 그림(아래 완료 기록
   v0.31 참고). `gonggi_stones.gd`/`jar_stamp.gd`도 이미 실제
   스프라이트로 교체됨.
2. **`scenes/chapter1_end.tscn`~`chapter3_end.tscn`은 v0.26으로 전부
   최소한의 임시 종료 화면인 채로 챕터 2/3까지 연결 완료**(챕터 1/2
   종료는 Enter로 일상 복귀, 챕터 3(마지막)은 정적 임시 엔딩 화면).
   사람이 실제 엔딩 연출/각 종료 화면 콘텐츠를 더 채워 넣고 싶다면 그건
   스토리/설계 결정 영역이니 지시를 기다릴 것.
3. 각 신규 씬/상호작용 작업 후 `qa/run_all.sh`(시각) + `tests/run_all.sh`
   (상호작용, 해당하면) 로 검증 → 사람 눈 확인은 여전히 최종 기준.
4. **맵 확장은 현실+꿈 둘 다 v0.23/v0.24로 완료**(사람 피드백, 2026-09-08
   + 2026-09-14 "레벨 디자인 및 내부 구조는 임의로 일임한다" — 기와집
   내부 방 2개, 실제 스크롤링도 처음으로 검증 완료, DESIGN.md §8.3/
   STATUS.md v0.23·v0.24 참고). 새 방 안에는 가구/상호작용 오브젝트가
   전혀 없음(순수 이동 공간만 확보) — 필요해지면 채울 것.
5. **재미 요소 추가(사람 피드백, 2026-09-08 — "인터렉션 오브젝트나 풋
   스위치 등, 무언가가 더 필요하다")는 v0.22로 완료**: 나무/덤불 조사
   (`scenery_flavor.gd`, v0.07)에 이어 풋 스위치도 배치함 —
   `scripts/pressure_plate_flavor.gd`(원두막 마루, 밟으면 분위기용
   대사 한 줄)로 챕터 1 꿈에 적용.

> **막혀있던 항목 전부 "기능부터, 내용은 나중" 방침으로 placeholder
> 처리 완료(v0.28→v0.31)**: 탈출 후 단서 콘텐츠(v0.29), 플레이어
> 캐릭터 스프라이트(v0.30), 마루/사방치기 아트(v0.31) — 전부 사람 확인
> (2026-09-15)에 따라 실제/최종 내용이 아니라 placeholder로 구조+시각
> 요소를 채워 넣었다. **각 항목의 "진짜 최종본"(단서가 실제로 무엇인지,
> 캐릭터 최종 디자인, 사방치기/판자를 진짜 아트로 교체할지 여부)은
> 여전히 사람 확인이 필요** — INBOX.md "처리 대기 중"의 캐릭터 스프라이트
> 항목 참고. 새 INBOX.md 지시가 오기 전까지는 기존 기능의 버그 수정/
> 리팩터/테스트 보강 위주로 이어갈 것.
...
- **v0.31 — 마루 밑 판자/사방치기를 `ColorRect` 그레이박스에서 실제
  ...
  진행 확인). 두 오브젝트 모두 `assets/props/main_tileset_props/
  README.md`에 "이 팩엔 정확히 맞는 소재가 없다"고 이미 기록돼 있던
  것들이라, 새 에셋 팩을 구하는 대신 **직접 제작**으로 대체:
  - `floorboard.gd`(마루 밑 판자): 이미 마루 바닥에 쓰고 있는 타일
    (`A5.png`의 `WOOD_LIGHT`)을 그대로 크롭한 뒤 살짝 어둡게+위/왼쪽에
    그림자 라인을 더해 "들려 있는 판자"처럼 보이게 가공한
    `floorboard_loose.png`로 교체 — 새 그림을 그린 게 아니라 이미
    라이선스 확인된 기존 타일을 가공.
  - `hopscotch_key.gd`(사방치기): 맞는 타일 자체가 아예 없어서,
    `scripts/hopscotch_visual.gd`(신규)가 `_draw()`로 사각형 4칸 +
    반원(하늘칸)을 직접 벡터로 그림. 사람 얼굴/몸처럼 복잡한 형태가
    아니라 사각형+반원 조합이라 절차적으로 정확하게 그릴 수 있는
    소재라서 채택 — 캐릭터 스프라이트 때 중단했던 "도형 조합으로 사람을
    흉내내는" 절차적 생성과는 성격이 다름(사람 형상은 실패, 기하학적
    도형은 안정적으로 가능).
  `chapter1_dream.tscn`의 두 노드(`Floorboard/Visual`, `HopscotchKey/
  Visual`)를 각각 `Sprite2D`/스크립트 붙은 `Node2D`로 교체.
  `tests/run_all.sh`(30개, 전부 통과 — 두 스크립트 다 인터랙션 로직은
  손대지 않고 순수 시각 요소만 교체해서 회귀 없음) + `qa/run_all.sh`
  (11개, 전부 통과) + 창을 띄운 일회성 확대 캡처로 두 그림 모두 눈으로
  확인(판자는 주변 마루보다 눈에 띄게 어둡고 그림자 틈이 보임, 사방치기는
  흙바닥 위에 분필 선처럼 또렷하게 보임) 후 검증 스크립트 삭제.
...
- **v0.29 — 탈출 후 일상 파트 "단서" 오브젝트 추가(구조 먼저, 내용은
  ...
  결정이 필요해서 계속 보류였지만, 사람이 이번엔 구조부터 임의 내용으로
  채워도 된다고 확인함). 새 범용 컴포넌트 `scripts/clue_object.gd`
  (`InteractableBase` 상속, `scenery_flavor.gd`와 같은 패턴) —
  `required_chapter`를 탈출하기 전엔 안 보이고 상호작용도 안 되다가,
  `ChapterProgress.current_chapter`가 그 값을 넘으면(`_is_visible_now()`
  오버라이드) 나타나서 지정한 대사를 보여준다. 챕터 1의 완료도
  `floorboard.gd`가 `ChapterProgress.current_chapter`를 2로 올리는
  식으로 편입돼 있어서, 이 하나의 값으로 챕터 1/2/3 전부의 "탈출 여부"를
  판단할 수 있었다(별도로 `Chapter1Progress`를 볼 필요 없음).
  `chapter1_real.tscn`(일상 허브)에 `ClueChapter1`(밭, 1장 탈출 후)/
  `ClueChapter2`(마루 밑, 2장 탈출 후) 2개 배치 — 대사는 전부
  placeholder이고 그렇다고 명시적으로 라벨링함. 새 테스트
  `tests/test_clue_objects.gd`(14개 assert)로 탈출 전 비가시/무반응,
  탈출 후 가시/정확한 대사 표시를 검증. `tests/run_all.sh`(30개 파일,
  전부 통과) + QA 스크린샷 확인(초기 상태는 안 보이므로 시각적 변화
  없음, 정상). **주의**: "단서"가 실제로 무엇을 의미하는지, 나중에
  다른 시스템과 어떻게 연결될지는 여전히 미정 — 지금은 상호작용
  틀만 있고 내용은 사람 확인 후 갈아끼울 것.
...
- **v0.27 — 일상 파트 NPC 3개 추가**(INBOX.md 2026-09-14, DESIGN.md
  ...
  (384,-160, 안방 내부)/`NpcB`(800,-160, 건넌방 내부)/`NpcC`(960,544,
  마당 원두막 근처). 대사는 전부 placeholder(예: "오늘도 덥네.",
  "또 왔구나. 밥은 먹었니?", "...") — 사람 피드백대로 "틀을 먼저 만들고
  디테일은 추후". 새 테스트 `tests/test_daily_life_npcs.gd`(16개
  assert)로 세 NPC 모두 도달 가능하고 각자 지정한 대사가 뜨는지 검증 —
  NPC 자신이 `blocks_movement` 그룹이라 다가가면 자동으로 한 칸 앞에서
  막히면서 facing도 그 방향으로 갱신되는 특성(`player.gd`: 막혀도
  facing은 갱신됨)을 이용해 정확한 정지 좌표를 미리 계산할 필요 없이
  자연스럽게 인접+정면 상태를 얻음. `tests/run_all.sh`(29개 파일, 전부
  통과) + `qa/run_all.sh`(chapter1_real 스크린샷 육안 재확인, 새 NPC
  3개 모두 정상 렌더링) 검증 완료. 이로써 DESIGN.md §11(챕터 2/3 + 일상
  NPC 3개) 계획이 전부 구현됨.
...
- **v0.24 — 챕터 1 꿈에도 기와집 내부 반영(DESIGN.md §4 "같은 물리
  ...
  `chapter1_dream.tscn`의 `CollisionMap.blocked_rects`/
  `Camera2D.limit_top`/`Player.play_area`도 현실과 동일하게 갱신.
  안방/건넌방 위치(cols9-30, rows-10~-1)와 안방-건넌방 사이 문(col19,
  row-5,-4)은 현실과 완전히 동일 — DESIGN.md §4의 "같은 물리 공간이
  현실/꿈 모두에 반복 등장" 원칙을 지킨다. **출입문 위치만 예외**:
  현실의 문 자리(col14-15, x448-511)는 꿈 씬에서는 이미 그 자리에 있는
  `Floorboard`(480,160)/`GonggiStones`(416,160)와 겹쳐서 그대로 못
  쓰고, col10-11(x320-383, 안방 서쪽 구석)로 옮김 — 꿈은 "객관적인
  과거를 그대로 재생하는 공간이 아니다"(DESIGN.md §4.2)는 원칙에 비춰
  이 정도 미세한 차이는 허용 범위로 판단. 분위기 오버레이
  `InteriorTint`(차가운 청회색, Color(0.2,0.22,0.28,0.35))를 안방/
  건넌방 전체에 추가해서 다른 구역들처럼 꿈 특유의 색감을 반영. **버그
  발견 및 수정**: `Minimap.show_map()`을 호출하는 두 진입 스크립트
  (`chapter1_real_entry.gd`/`chapter1_dream_entry.gd`)가 v0.23에서 맵
  범위 갱신을 안 해서(여전히 `Rect2(0,0,1280,720)`), 집 안에서는
  미니맵 점이 맨 위에 붙은 채 정확한 위치를 못 보여주고 있었다 — 둘 다
  `Rect2(0,-320,1280,1040)`로 수정. 신규 회귀 테스트
  `tests/test_house_interior_dream.gd`(현실 쪽과 같은 구조를 꿈의 문
  위치 기준으로 재검증, `Floorboard`/`GonggiStones`를 우회하는 이동
  경로 필요 — 6개 어서션). 일회성 검증 스크립트로 실제 창 캡처 확인
  (검증 중 카메라 스무딩이 완전히 따라잡는 데 순간이동 직후 10프레임
  으로는 부족해서 120프레임으로 늘려야 했음 — 실제 플레이에서는
  플레이어가 서서히 이동하므로 문제 없음, 스크립트 검증 한정 이슈).
  전체 27종 + QA 7개 씬 재통과. **아직 안 한 것**: 방 안에 가구/
  상호작용 오브젝트 없음(v0.23과 동일하게 순수 이동 공간만 확보).
- **v0.23 — 챕터 1 현실 맵 확장: 기와집 내부 방 2개(INBOX.md 2026-09-14,
  레벨 디자인 위임, DESIGN.md §8.3)**: 사람 피드백(2026-09-08 "집도
  돌아다닐 수 있을만큼 넓고, 방도 두 개 정도 더 만들자",
  2026-09-14 "레벨 디자인 및 내부 구조는 임의로 일임한다")에 따라 방
  배치를 직접 정해서 구현. **기존 마당/툇마루/밭/원두막은 좌표 그대로
  전혀 안 건드림** — `chapter1_real_background.gd`에 새 타일 채우기만
  추가: 툇마루 쪽 외벽(col14-15, x448-511)에 문을 뚫고, 그 위(북쪽,
  row-10~-1, y-320~0)에 내부 바닥(안방+건넌방)을 새로 그림. 안방(서쪽,
  col10-18)과 건넌방(동쪽, col20-29) 사이는 벽 하나(col19)로 나뉘고
  가운데 2칸(row-5,-4, y-160~-96)만 뚫려 있어 그리로만 오갈 수 있다.
  `CollisionMap.blocked_rects`도 그대로 반영(기존 외벽 rect 하나를 문
  자리만큼 둘로 쪼갬 + 새 내부 벽 5개 rect 추가). **맵이 화면(1280×720)
  보다 커진 첫 사례**라 `Camera2D.limit_top`을 0→-320으로,
  `player.gd`의 `play_area`도 씬별로 `Rect2(0,-320,1280,1040)`로
  오버라이드 — 지금까지 이 인프라(limit_* 기반 스크롤링 준비)는 깔려만
  있고 한 번도 실제로 스크롤이 검증된 적이 없었다(STATUS.md에 여러 번
  기록된 미검증 사항). 일회성 검증 스크립트로 실제 창을 띄워 이동
  전/후 스크린샷을 비교해서 카메라가 실제로 따라 올라오는지, 두 방과
  가운데 문이 시각적으로 정확히 렌더링되는지 확인(검증 후 스크립트/
  캡처는 삭제, 프로젝트 관례). 신규 회귀 테스트
  `tests/test_house_interior.gd`(출입문 통과, 안방→건넌방 문 통과,
  북쪽 벽 차단, 안방-건넌방 사이 벽이 문 아닌 자리에서는 차단 — 9개
  어서션). `tests/test_movement_bounds.gd`의 카메라 limit 어서션도 새
  값(0,-320,1280,720)에 맞게 갱신. **방 안에는 가구/상호작용 오브젝트
  없음**(순수 이동 공간 확보가 목적, DESIGN.md §8.3 참고) — 필요해지면
  나중에 채울 것. **아직 안 한 것**: `chapter1_dream.tscn`(꿈)에는 이
  확장을 반영 안 함 — 이번 이터레이션 범위 밖(DESIGN.md §8.3 참고).
  전체 26종 + QA 7개 씬 재통과.
...
- **v0.21 — 미니맵 추가(INBOX.md 2026-09-14)**: 새 오토로드 `Minimap`
  ...
  점으로 표시한다. `show_map(map_bounds: Rect2)`를 호출하면 켜지고
  `hide_map()`으로 끈다(오토로드라 명시적으로 꺼야 이전 씬 상태가 안
  남음). **켜는 곳**: `chapter1_real.tscn`(새 진입 스크립트
  `scripts/chapter1_real_entry.gd` 신설, 이 씬은 지금까지 루트
  스크립트가 없었음)과 `chapter1_dream_entry.gd`(기존 "1장 시작" 타이틀
  로직에 추가). **끄는 곳**: `intro_sequence.gd`(프롤로그 — 순수
  나레이션), `chapter1_end.gd`(챕터 종료 — 정적 화면). **챕터 타이틀
  카드 연동**: `Minimap._process()`가 `ChapterTitleCard.is_active()`도
  함께 확인해서, "1장"/"1장 종료" 타이틀이 떠 있는 챕터 전환 순간에는
  자유 이동 씬 안에 있어도 자동으로 같이 숨는다("챕터 전환이나
  나레이션... 플레이 불가능한 시점" 요구사항의 두 절반을 각각 씬
  단위/전역 상태 단위로 처리). NPC 대화·아이템 팝업 중에는 계속
  보임(그 정도는 "플레이 불가능"으로 보지 않음). 신규 회귀 테스트
  `tests/test_minimap.gd`(프롤로그에서 숨김, 현실/꿈에서 보임, 이동 시
  점 위치 갱신, 타이틀 카드 중 숨김→해제 후 재표시, 챕터 종료에서
  숨김 — 10개 어서션). 전체 24종 + QA 7개 씬 재통과, 실제 창 캡처로
  렌더링 확인.
...
- **v0.19 — 챕터 1 전체 경로 통합 검증 강화(코드 변경 없음)**:
  ...
  뜨는지까지 확인) → 퍼즐 자유 순서로 완료 → 일기 개봉("1장 종료" 카드를
  거쳐) → 챕터 종료 화면 → **Enter로 다시 현실(일상)까지 돌아오는 것**
  까지 한 번에 이어지는지 11개 어서션으로 검증(기존 9개에서 확장). 이번
  이터레이션은 새 기능 추가가 아니라 지금까지 5번의 이터레이션(v0.15
  퍼즐 구조 결정 → v0.16 실제 이전 → v0.17 타이틀 카드 → v0.18 종료→
  현실 연결)에 걸쳐 만든 챕터 1의 새 구조가 실제로 끊김 없이 전부
  맞물려 돌아가는지 마지막으로 한 번에 확인하는 마무리 검증. 문제 없이
  통과 — 전체 22종 + QA 7개 씬 재통과.
- **v0.18 — 챕터 종료 화면에서 현실(일상)로 이어지는 연결 추가
  (INBOX.md 2026-09-09, 부분 처리)**: `chapter1_end.tscn`이 지금까지는
  텍스트 한 줄짜리 막다른 화면이었는데, 새 스크립트 `scripts/
  chapter1_end.gd`를 붙여서 `ui_accept`를 누르면 `chapter1_real.tscn`
  (현실)으로 돌아가도록 연결했다("챕터 종료한 뒤 일상이 시작된다" 요구
  사항의 구조적 절반). 화면에 "Enter: 계속" 안내 문구 추가. **의도적으로
  안 한 것**: 챕터 종료 이후를 위한 새 NPC 대사나 "단서" 콘텐츠는 전혀
  추가하지 않음 — 기존 `ShadowNPC`("[...]")와 나무/덤불 조사 대사를
  그대로 재사용할 뿐이다. INBOX.md "탈출한 후... 단서를 얻을 수 있는
  일상 플레이" 항목이 요구하는 실제 단서/대사 콘텐츠는 사람이 구체
  내용을 정해줘야 만들 수 있어서 여전히 미착수 — 이번엔 그 콘텐츠를
  놓을 수 있는 통로(현실로 돌아가는 구조)만 만들었다. 신규 회귀 테스트
  `tests/test_chapter1_end_continue.gd`(종료 화면 로드 확인 + Enter로
  현실 전환 확인, 2개 어서션). 전체 22종 + QA 7개 씬 재통과, 실제 창
  캡처로 "Enter: 계속" 문구 렌더링 확인.
...
- **v0.15 — 챕터 1 퍼즐 구조 변경 결정을 DESIGN.md에 기록(코드 변경
  ...
  꿈에 진입하면 챕터가 시작되고, 꿈에 탈출하면 챕터가 종료된다")를
  `docs/DESIGN.md` §4.1(박스 노트 추가)과 §8.1(전면 재작성)에 반영.
  **핵심 변경**: 4단계 퍼즐(마루 밑/사방치기 열쇠/장독 나무패/일기
  펼치기)이 이제 순환(낮잠↔각성)마다 하나씩이 아니라 **꿈 방문 한 번
  안에서 전부** 진행된다 — 1~3단계는 자유 순서, 4단계(일기)는 열쇠+
  나무패가 있어야 열림. 일기가 열리면 그게 곧 "탈출"이자 "챕터 종료".
  **이 결정은 사람이 직접 지시한 구조 변경이지 내가 새로 지어낸 스토리/
  레벨 디자인이 아님** — 기존에 만든 퍼즐 오브젝트/대사는 전부 그대로
  재사용하고, 그것들이 "어디(현실이냐 꿈이냐)에서, 어떤 페이싱으로"
  등장하는지만 바뀐다. 실제 코드(오브젝트를 chapter1_dream.tscn으로
  옮기기, stage 게이팅 제거, WakeTrigger 역할 재검토, 챕터 시작/종료
  타이틀 카드)는 다음 버전(v0.16~)에서 이어감 — **이 버전 시점에는
  아직 게임 동작이 하나도 안 바뀌었음**(문서만 변경, 전체 22종 테스트
  는 그대로 무변경 통과 상태).
- **v0.14 — 개발자용 챕터 이동 치트 추가(INBOX.md 2026-09-09)**:
  새 오토로드 `DebugWarp`(`scripts/debug_warp.gd` +
  `scenes/common/DebugWarp.tscn`) — F9로 메뉴를 열고 닫고, 열린 상태에서
  숫자 키(1/2/3...)로 `CHECKPOINTS` 배열의 항목을 골라 그 씬으로
  `change_scene_to_file()`, 동시에 `Chapter1Progress`(stage/has_key/
  has_stamp/diary_opened)를 그 체크포인트의 값으로 덮어쓴다("이동하면
  현재 진행도를 날리고, 해당 진행도 시작지점에 보유하고 있어야하는
  아이템만 지니게 한다"는 지시 그대로). **다른 인터랙션 스크립트들과
  달리 `DialogueSystem.is_active()` 등 어떤 상태 체크도 안 함** —
  "모든 상황에서 접근 가능"이 요구사항이라 의도적으로 뺐다(테스트로
  대화창이 열린 상태에서도 F9가 먹히는지 직접 확인). `OS.is_debug_build()`
  가 false면(배포 빌드) 아예 반응하지 않아 플레이어에게 노출되지
  않는다. **지금은 챕터 1까지만 있어서 체크포인트도 3개뿐**(프롤로그 /
  1장 시작 / 1장 종료) — INBOX.md의 "챕터 2개 추가"가 실제로 구현되면
  그 체크포인트들을 `CHECKPOINTS` 배열에 추가하기만 하면 된다(그
  전까지는 미완성 상태). 신규 회귀 테스트 `tests/test_debug_warp.gd`
  추가(11개 어서션 — 메뉴 열기/닫기, 진행 상태를 일부러 어질러 놓은 뒤
  체크포인트 이동 시 정확히 그 지점 값으로 초기화되는지, 대화 중에도
  반응하는지). `tests/run_all.sh`가 이제 22개 파일을 실행함. 전체
  22종 + QA 7개 씬 재통과(치트 메뉴 자체는 기본으로 닫혀 있어서 QA
  스크린샷에는 영향 없음 — 개발자 전용 화면이라 시각 캡처로 별도 확인은
  생략).
...
- **v0.07 — 마당 나무/덤불 조사 상호작용 추가("재미 요소" 대응 일부)**:
  ...
  여러 오브젝트에 재사용 가능, `gonggi_stones.gd` 패턴과 동일한 접근)를
  신설하고, `chapter1_real.tscn`의 기존 데코 스프라이트(`TreeGreen`,
  `TreeSmall`, `BushBerry`) 위치에 정확히 겹치는 투명 `Node2D` 트리거
  3개(`TreeGreenExamine`/`TreeSmallExamine`/`BushBerryExamine`)를 추가,
  각각 짧은 환경 묘사 대사 하나씩 부여. 새 진상/퍼즐/레벨 디자인 결정
  없이 순수 분위기 텍스트만 추가한 것 — INBOX.md "인터렉션 오브젝트나
  풋 스위치 등, 무언가가 더 필요하다" 피드백에 대한 안전한 대응 하나
  (풋 스위치 자체(`pressure_plate.gd`) 배치는 여전히 레벨 디자인 결정이
  필요해 보류 중, 아래 "다음 할 일 큐" 참고). 새 노드는 기존 z-order
  규칙대로 `YSortObjects` 앞에 배치(안 그러면 플레이어보다 위에 그려짐 —
  이전 이터레이션에서 겪은 버그와 같은 종류). 각 트리거 위치가
  `CollisionMap.blocked_rects` 밖의 그리드 정렬 위치에서 `interact_radius`
  (28px) 안에 들어오는지 좌표로 직접 검증(TreeGreen: (704,448)에서 거리
  22.4px, TreeSmall: (96,672)에서 12.6px, BushBerry: (1152,640)에서
  20.1px). 신규 자동 테스트 `tests/test_scenery_flavor.gd` 추가(플레이어를
  실제로 TreeGreen 옆까지 이동시켜 Enter로 조사, 지정한 대사가 뜨는지
  확인, 5개 어서션) — `tests/run_all.sh`가 이제 18개 파일을 실행함. 전체
  18종 + QA 7개 씬 재통과, 실제 창 캡처로 씬 렌더링에 시각적 이상 없음도
  확인(트리거가 투명 노드라 프롬프트 라벨 외에는 아무것도 새로 안 보임,
  플레이어가 없을 때는 라벨도 숨겨진 채로 정상).
- **v0.06 — 벽 낙서 flash() "사라지는 쪽" 테스트 커버리지 보강**:
  `wall_mark_flash.gd`의 flash()는 지금까지 "visible=true로 켜지는
  순간"만 테스트했고(동기적이라 타이밍 안전), `flash_seconds` 뒤
  다시 숨겨지는 쪽은 실제 시간 경과에 의존해서 테스트한 적이 없었다.
  v0.02/v0.04/v0.05에서 애니메이션 검증에 쓴 것과 같은 패턴
  (`create_timer`로 충분히 기다린 뒤 확인)을 적용해 `test_chapter1_
  puzzle.gd`에 어서션 추가. 코드 변경 없음, 전체 17종 + QA 7개 씬
  재통과.
- **v0.05 — v0.02/v0.04 애니메이션에 정식 회귀 테스트 추가**:
  `tools/verify_*.gd` 일회성 스크립트로만 확인하고 지웠던 아이템 팝업
  등장 애니메이션과 목표 힌트 페이드를 `tests/test_item_popup_unit.gd`
  (5개 어서션 — 등장 직후 스케일/알파가 최종값 미만, 애니메이션 종료 후
  정확히 1.0 안착)와 `tests/test_objective_hint_unit.gd`(6개 어서션 —
  초기 페이드 완료, 내용 불변 시 재발동 안 함, 내용 변경 시 알파가
  다시 내려갔다 올라옴)로 정식 편입. `tests/run_all.sh`가 이제 17개
  파일을 실행함. 전체 재통과 + QA 7개 씬도 재확인.
- **v0.04 — 목표 힌트 텍스트 전환 페이드("손맛" 개선 3)**:
  `objective_hint.gd`가 이제 힌트 문구가 실제로 바뀔 때만(매 프레임이
  아니라) `modulate.a`를 0→1로 페이드시키며 갈아끼운다(0.25초). 대화창/
  팝업 때문에 숨겨졌다 다시 나타나는 경우는 페이드 대상이 아니고(그냥
  즉시 보임), 순수하게 "내용이 바뀌는 순간"에만 적용됨. `.text`는
  즉시 새 값으로 설정되므로 기존 어서션(즉시 비교)에는 영향 없음 —
  v0.02/v0.03과 같은 안전 패턴. 전체 15종(무변경) + QA 7개 씬 재통과.
