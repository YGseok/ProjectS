...
2. 각 신규 씬/상호작용 작업 후 `qa/run_all.sh`(시각) + `tests/run_all.sh`
...
> 위 1번이 사람의 후속 조치(스토리 결정) 대기 상태다. 세 번의 이터레이션
> 배치(테스트 9종, QA/테스트 일괄 실행 스크립트, 문서 감사, STEP/ 정리)를
> 거치며 "결정 없이 할 수 있는
> 품질/커버리지 보강"은 상당히 소진됐다 — 다음 세션은 새로 발견되는
> 버그가 있으면 그걸 고치되, 억지로 새 테스트/정리 거리를 만들어내기보다
> **1번(스토리 결정)이 왔는지 INBOX.md/docs/scenario/부터 확인**하는
> 게 우선이다. `Outside.png`(자연 오브젝트)는 사용 시작함(나무/덤불 3종,
